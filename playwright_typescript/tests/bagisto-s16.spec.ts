 ...
 */
test.describe('Bagisto S16 - Concurrent Place Order (Race Condition)', () => {
  test('S16 – Two browsers place order at EXACTLY same time', async ({ page, context }) => {
    test.setTimeout(300000); // 5 minutes - allow infinite wait for both browsers
    
    console.log('');
    console.log('='.repeat(80));
    console.log('S16B: CONCURRENT PLACE ORDER - Testing Race Condition for Duplicate Orders');
    console.log('='.repeat(80));
    console.log('');
